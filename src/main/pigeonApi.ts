...
async function call<T>(path: string, method = 'GET'): Promise<T> {
  ...
  const res = await fetch(BASE + path, { method, headers: { 'x-api-key': key } });
...
export const pigeon = {
  ...
  /** Fetching the detail is what marks it read server-side. */
  markRead: (id: string) => call<unknown>(`/emails/${encodeURIComponent(id)}`),
  markUnread: (id: string) => call<unknown>(`/emails/${encodeURIComponent(id)}/unread`, 'POST'),
  deleteMail: (id: string) => call<unknown>(`/emails/${encodeURIComponent(id)}`, 'DELETE'),
};
